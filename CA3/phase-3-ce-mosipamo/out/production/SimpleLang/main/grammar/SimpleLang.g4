...
//    | LeftParen typ1 = typeName { $expressionRet.setTypeName($typ1.typeNameRet); $expressionRet.setLine($ctx.getStart().getLine()); } RightParen LeftBrace i = initializerList { $expressionRet.setInitializerList($i.initializerListRet); $expressionRet.setLine($ctx.getStart().getLine()); } Comma? RightBrace
    | LeftParen typ1 = typeName {
          PtrExpression ptr = new PtrExpression();
          ptr.setTypeName($typ1.typeNameRet);
          ptr.setLine($ctx.getStart().getLine());
      } RightParen LeftBrace i = initializerList {
          ptr.setInitializerList($i.initializerListRet);
          ptr.setLine($ctx.getStart().getLine());
          $expressionRet = ptr;
      } Comma? RightBrace
    ...
    | e2 = expression { FunctionCallExpression fce = new FunctionCallExpression(); fce.setExpression($e2.expressionRet); } l=LeftParen (a = argumentExpressionList { fce.setArgumentExpressionList($a.argumentExpressionListRet); fce.setLeft_par($l.text); fce.setLine($ctx.getStart().getLine()); })? r=RightParen {fce.setRight_par($r.text); $expressionRet = fce;}                      // Function call
    ...
    (                                         // Prefix operators (zero or more)
       ...
       | LeftParen typ = typeName { PtrExpression ptr = new PtrExpression(); ptr.setTypeName($typ.typeNameRet); ptr.setLine($ctx.getStart().getLine());} RightParen LeftBrace i1 = initializerList { ptr.setInitializerList($i1.initializerListRet); ptr.setLine($ctx.getStart().getLine()); $expressionRet = ptr;} Comma? RightBrace
       ...
       | Sizeof LeftParen typ2 = typeName { PtrExpressionWithoutInitializerList ptr2 = new PtrExpressionWithoutInitializerList(); ptr2.setTypeName($typ2.typeNameRet); ptr2.setLine($ctx.getStart().getLine()); $expressionRet = ptr2; } RightParen
    )
...
| LeftParen typ3 = typeName RightParen ca = castExpression {
      $expressionRet = $ca.castExpressionRet;
      $expressionRet.setTypeName($typ3.typeNameRet);
      $expressionRet.setLine($ctx.getStart().getLine());
  }
...
parameterList returns [ParameterList parameterListRet]
    ...
    (Comma pp=parameterDeclaration {$parameterListRet.addParameterDeclaration($pp.parameterDeclarationRet); $parameterListRet.setLine($ctx.getStart().getLine());})* ;
...
abstractDeclarator returns  [AbstractDeclarator abstractDeclaratorRet]
    :
    p=pointer { $abstractDeclaratorRet = new AbstractDeclarator(); $abstractDeclaratorRet.addPointerType($p.pointerTypeRet); $abstractDeclaratorRet.setLine($ctx.getStart().getLine());}
    |{$abstractDeclaratorRet = new AbstractDeclarator();} (pp=pointer  { $abstractDeclaratorRet.addPointerType($pp.pointerTypeRet); $abstractDeclaratorRet.setLine($ctx.getStart().getLine());} )?
...
initializer returns [Initializer initializerRet]
    : e=expression {$initializerRet = new Initializer(); $initializerRet.setExpression($e.expressionRet); $initializerRet.setLine($ctx.getStart().getLine());}
    | LeftBrace i=initializerList Comma? RightBrace {$initializerRet = new Initializer(); $initializerRet.setInitializerList($i.initializerListRet); $initializerRet.setLine($ctx.getStart().getLine());} ;
...
jumpStatement returns [JumpStatement jumpStatementRet]
    ...
    ( key=(Continue | Break | Return) (e=expression{$jumpStatementRet.setExpression($e.expressionRet); $jumpStatementRet.setKeyword($key.text); $jumpStatementRet.setLine($ctx.getStart().getLine());})? ) Semi ;
