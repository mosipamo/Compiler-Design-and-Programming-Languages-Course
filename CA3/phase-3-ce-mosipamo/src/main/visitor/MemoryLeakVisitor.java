...
public class MemoryLeakVisitor extends Visitor<Void> {
    private static final Set<String> builtIns = Set.of("printf", "scanf", "malloc", "free", "realloc", "calloc");
    public static final Map<String, Integer> pointersUsed = new HashMap<>();
//    private final Map<String, Set<String>> functionFrees = new HashMap<>();
    private final Map<String, Set<Integer>> functionFrees = new HashMap<>();
    private final Map<String, Integer> currentParameterPositions = new HashMap<>();
    ...
    public Void visit(InitDeclarator initDeclarator) {
        ...
        if (initDeclarator.getDeclarator() != null) {
            ...
            if (declarator.getDirectDeclarator() != null) {
                ...
                pointerName = directDeclarator.getIdentifier();
        ...
        if (initDeclarator.getInitializer() != null) {
            ...
            if (i.getExpression() != null) {
                Expression exp = i.getExpression();
                if (exp instanceof FunctionCallExpression) {
                    if (((FunctionCallExpression) exp).getExpression() != null) {
                        Expression expr2 = ((FunctionCallExpression) exp).getExpression();
                        if (expr2 instanceof IdentifierExpression) {
                            String pointerKind = ((IdentifierExpression) expr2).getName();
                            if (pointerKind.equals("malloc") || pointerKind.equals("realloc") || pointerKind.equals("calloc")) {
                                this.pointersUsed.put(pointerName, initDeclarator.getLine());
                            }
                        }
                    }
                }
    ...
    public Void visit(ParameterDeclaration parameterDeclaration) {
        ...
        if (parameterDeclaration.getDeclarator() != null) {
            ...
            if (d.getDirectDeclarator() != null) {
                ...
                if (!currentParameterPositions.containsKey(name)) {
                    currentParameterPositions.put(name, currentParameterPositions.size());
                }
                try {
                    SymbolTable.top.put(varItem);
                } catch (ItemAlreadyExistsException e) {
    ...
    public Void visit(AssignmentExpression assignmentExpression) {
        ...
        if (assignmentExpression.getLeft() != null) {
            Expression expression = assignmentExpression.getLeft();
            if (expression instanceof IdentifierExpression) {
                pointerName = ((IdentifierExpression) expression).getName();
            }
        ...
        if (assignmentExpression.getRight() != null) {
            ...
            if (expression instanceof FunctionCallExpression) {
                Expression an = ((FunctionCallExpression) expression).getExpression();
                if (an instanceof IdentifierExpression) {
                    if (((IdentifierExpression) an).getName().equals("malloc") || ((IdentifierExpression) an).getName().equals("realloc") || ((IdentifierExpression) an).getName().equals("calloc")) {
                        this.pointersUsed.put(pointerName, expression.getLine());
                    }
                }
            }
    ...
    public Void visit(FunctionDefinition functionDefinition) {
        ...
        if (functionDefinition.getDeclarator() != null) {
            ...
            if (declarator.getDirectDeclarator() != null) {
                DirectDeclarator directDeclarator = declarator.getDirectDeclarator();
                if (directDeclarator.getDirectDeclarator() != null) {
                    DirectDeclarator directDeclarator1 = directDeclarator.getDirectDeclarator();
                    funcName = directDeclarator1.getIdentifier();
                }
            }
        }
        functionFrees.put(funcName, new HashSet<>());
    ...
    public Void visit(FunctionCallExpression functionCallExpression) {
        ...
        if (functionCallExpression.getExpression() != null) {
            ...
            if (exp instanceof IdentifierExpression) {
                            ...
                            for (Expression ee: expr) {
                                if (ee instanceof IdentifierExpression) {
                                    ...
                                    try {
                                        ...
                                        this.pointersUsed.remove(pointerName);
                                        if (currentParameterPositions.containsKey(pointerName)) {
                                            int pos = currentParameterPositions.get(pointerName);
                                            functionFrees.get(currentFunction).add(pos);
                                        }
                                    } catch (ItemNotFoundException e) {
                ...
                if (functionFrees.containsKey(pointerKind)) {
                    Set<Integer> freedParamIndexes = functionFrees.get(pointerKind);
                    ArgumentExpressionList argList = functionCallExpression.getArgumentExpressionList();
                    if (argList != null) {
                        ArrayList<Expression> args = argList.getExpression();
                        for (int i = 0; i < args.size(); i++) {
                            Expression arg = args.get(i);
                            if (arg instanceof IdentifierExpression idExpr) {
                                String varName = idExpr.getName();
                                if (freedParamIndexes.contains(i)) {
                                    pointersUsed.remove(varName);
                                }
                            }
                        }
                    }
    ...
    public void chap() {
        for (String key : this.pointersUsed.keySet()) {
            System.out.println("Line:" + this.pointersUsed.get(key) + " -> memory not deallocated");
        }
    }
}