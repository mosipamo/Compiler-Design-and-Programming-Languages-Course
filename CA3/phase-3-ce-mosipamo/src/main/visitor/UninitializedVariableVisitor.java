...
public class UninitializedVariableVisitor extends Visitor<Void> {
    private static final Set<String> builtIns = Set.of("printf", "scanf", "malloc", "free", "realloc", "calloc");

    private final Map<String, Integer> uninitializedVars = new HashMap<>();
        ...
        if (initDeclarator.getDeclarator() != null) {
            ...
            if (declarator.getDirectDeclarator() != null) {
                ...
                if (directDeclarator.getDirectDeclarator() != null) {
                    ...
                    int line = initDeclarator.getLine();
                    if (initDeclarator.getInitializer() == null) {
                        uninitializedVars.put(name, line);
                    }
                    else {
                        initializedVars.add(name);
                        uninitializedVars.remove(name);
                    }
                    try {
    ...
    public Void visit(IdentifierExpression identifierExpression) {
        ...
        int line = identifierExpression.getLine();
        ...
        if (!initializedVars.contains(name) && uninitializedVars.containsKey(name)) {
            System.out.println("Line:" + line + " -> uninitialized variable used");
        }
        return null;
    }
    ...
    public Void visit(AssignmentExpression assignmentExpression) {
        ...
        if (assignmentExpression.getLeft() != null) {
            ...
            if (expression instanceof IdentifierExpression) {
                ...
                initializedVars.add(pointerName);
                uninitializedVars.remove(pointerName);
            }
    ...
    public Void visit(Declaration declaration) {
        if (declaration.getDeclarationSpecifiers() != null) {
            DeclarationSpecifiers declarationSpecifiers = declaration.getDeclarationSpecifiers();
            if (declarationSpecifiers.getSpecifiers() != null) {
                ArrayList<DeclarationSpecifier> declarationSpecifierArrayList = declarationSpecifiers.getSpecifiers();
                for (DeclarationSpecifier spec: declarationSpecifierArrayList) {
                    if (spec.getTypeSpecifier() != null) {
                        String varName = spec.getTypeSpecifier().getTypeName();
//                        System.out.println(varName);
                        ...
                        int line = spec.getLine();
                        if (!initializedVars.contains(varName)) {
                            uninitializedVars.put(varName, line);
                        }
                    }
                }
            }
                            ...
                            if (arg instanceof CastExpression unary && unary.getUnaryOperator() == UnaryOperator.AND) {
                                ...
                                if (inner instanceof IdentifierExpression idInner) {
                                    ...
                                    try {
                                        ...
                                        uninitializedVars.remove(varName);
...
//                    return null; // Done processing scanf, skip further analysis
                }
            }
