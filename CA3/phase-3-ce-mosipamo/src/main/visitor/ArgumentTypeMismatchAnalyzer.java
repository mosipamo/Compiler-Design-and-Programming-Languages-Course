...
public class ArgumentTypeMismatchAnalyzer extends Visitor<Void> {
...
//                System.out.println(typeVariableSymbolTableItem.getKey());
    ...
    private final List<String> typePrecedence = List.of(
    ...
    private boolean isCompatibleType(String expected, String actual) {
        ...
        int idxExpected = typePrecedence.indexOf(expected);
        int idxActual = typePrecedence.indexOf(actual);
        return idxActual != -1 && idxExpected != -1 && idxActual <= idxExpected;
    }
...
//                            System.out.println(SymbolTable.root.items);
//                            TypeFunctionDefinitionSymbolTable typeFunc = (TypeFunctionDefinitionSymbolTable) SymbolTable.root.getItem(name);
//
////                            if (typeFunctionDefinitionSymbolTable.getArgsType().get(0).equals(typeVariableSymbolTableItem.getReturnType())) {
////                                System.out.println("yes");
////                            }
////                            else {
////                                System.out.println("Line:" + functionCallExpression.getLine() + " -> argument type mismatch");
////                            }
...
public Void visit(FunctionCallExpression functionCallExpression) {
    ...
    if (functionCallExpression.getExpression() != null) {
        Expression exprName = functionCallExpression.getExpression();
        if (!(exprName instanceof IdentifierExpression)) return null;

        IdentifierExpression iExp = (IdentifierExpression) exprName;
        String funcName = iExp.getName();

        if (builtIns.contains(funcName)) return null;

        int argCount = 0;
        ArrayList<Expression> args = new ArrayList<>();
        if (functionCallExpression.getArgumentExpressionList() != null) {
            args = functionCallExpression.getArgumentExpressionList().getExpression();
            argCount = args.size();
        }

        String key = "Function_" + funcName + "#" + argCount;

        try {
            TypeFunctionDefinitionSymbolTable funcItem =
                    (TypeFunctionDefinitionSymbolTable) SymbolTable.root.getItem(key);

            ArrayList<String> expectedTypes = funcItem.getArgsType();

            for (int i = 0; i < args.size(); i++) {
                Expression argExpr = args.get(i);
                String inferredType = inferExprType(argExpr);
                String expectedType = expectedTypes.get(i);

//                System.out.println(expectedType);
//                System.out.println(inferredType);

                if (!isCompatibleType(expectedType, inferredType)) {
                    anses.add("Line:" + functionCallExpression.getLine() + " -> argument type mismatch");
                    break; // only report once per call
                }
            }

        } catch (ItemNotFoundException e) {
//            anses.add("Line:" + exprName.getLine() + "-> function \"" + funcName + "\" not declared");
        }
    }
