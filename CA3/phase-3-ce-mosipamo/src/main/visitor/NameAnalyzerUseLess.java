...
public class NameAnalyzerUseLess extends Visitor<Void> {
    ...
    private static final Vector<String> opizes = new Vector<>();
    ...
    private boolean isUsefulExpression(Expression expr) {
        return expr instanceof AssignmentExpression
                || expr instanceof FunctionCallExpression
                || (expr.getTemp_op() != null && !expr.getTemp_op().isEmpty());
    }
        ...
        for (BlockItem item : list) {
            ...
            if (item.getStatement() != null) {
                if (item.getStatement() instanceof ExpressionStatement) {
                    Expression ex = ((ExpressionStatement) item.getStatement()).getExpression();
                    if (ex != null) {
                        if (isUsefulExpression(ex)) {
                            new_blockItems.add(item);
